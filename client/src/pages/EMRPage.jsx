...
function EMRPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch appointments (prefers /api/appointments for current user; falls back to /patient/:id)
      try {
      // Preferred endpoint (returns array)
      ...
      // Fallback endpoint (returns { appointments })
      ...
      case 'overview':
        return (
          <div>
            ...
            {/* Recent Appointments (use bookedDate + timeSlot + doctor) */}
          ...
          {appointments.map((appointment, index) => (
            <div key={index} style={{
              ...
              <h3 style={{ margin: '0 0 10px 0', color: '#333' }}>
                {new Date(appointment.bookedDate || appointment.date).toLocaleDateString()}
                {appointment.timeSlot && ` -  ${appointment.timeSlot}`}
              </h3>
              <p style={{ margin: '5px 0', color: '#666' }}>
                <strong>Doctor:</strong> {appointment.doctor?.user?.name || appointment.doctor?.name || appointment.doctorName || 'N/A'}
