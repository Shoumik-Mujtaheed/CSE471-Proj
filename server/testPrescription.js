// server/testPrescription.js - Updated with correct database.js path
import dotenv from 'dotenv';
import connectDB from './config/database.js'; // ✅ FIXED - Updated path to config folder
...
// Import models with error handling (correct paths)
try {
  console.log('📦 Importing models...');
  const User = (await import('./models/User.js')).default;
  const Prescription = (await import('./models/Prescription.js')).default;
  const Inventory = (await import('./models/Inventory.js')).default;
  const { generateInvoiceForPrescription } = await import('./controllers/InvoiceController.js');
  
  console.log('✅ All imports successful');

  const testPrescription = async () => {
    try {
      // Use your existing database connection from config folder
      console.log('🔌 Connecting to MongoDB using config/database.js...');
      await connectDB();
      ...
      let medicines = await Inventory.find().limit(3);
      ...
      console.log('You can now check your database for the created prescription and invoice.');
      ...
      // Don't manually disconnect if using your database.js connection
      console.log('\n🔌 Database connection managed by config/database.js');
      process.exit(0);
    }
  };
